...
def cleanDir(maxFilesToDelete = 50):
	...
	for eachFile in firstSnapShot:
		if (eachFile[0:workingPathLen] == WorkingPath + "/LastPush/"):
			os.remove(eachFile)
...
try:
	...
	if("help" in ArgumentList):
		...
		elif("-dm" in ArgumentList or "-do" in ArgumentList):
			modelName += "_Debug_Model"
		else:
			modelName += "_Prod"
		#swithc device back to lower case
		device = device.lower()
